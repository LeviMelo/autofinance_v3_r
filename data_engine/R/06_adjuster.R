...
de_apply_adjustments <- function(universe_raw, events_apply, cfg) {
  ...
  # Optional price-like extras (if present in universe_raw)
  if ("average_raw" %in% names(dt)) dt[, average_adj_final := average_raw * adj_factor_final]
  if ("best_bid_raw" %in% names(dt)) dt[, best_bid_adj_final := best_bid_raw * adj_factor_final]
  if ("best_ask_raw" %in% names(dt)) dt[, best_ask_adj_final := best_ask_raw * adj_factor_final]
